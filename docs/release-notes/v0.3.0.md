# plutopy 0.3.0 — Finish-Up-A-Thon edition
...
- **End-to-end transpiler.** PLUTO source → Lark parse tree → readable
  ...
  (`plutopy.runtime` / `plutopy.async_runtime`).
- **CLI**: `plutopy parse | compile | run | demo | fmt | gen`.
- **Compile knobs**: `--runtime async`, `--style class`, `--emit json`,
  `--no-runtime` (inline). All compose.
- **Live TUI demo** (`plutopy demo`) — a fake satellite reacts to
...
The release wheel (`plutopy-0.3.0-py3-none-any.whl`) and sdist
(`plutopy-0.3.0.tar.gz`) are attached to this GitHub release. Both
pass `twine check`.

## Links

- [Docs](https://stzifkas.github.io/PLUTOpython/)
- [Playground](https://stzifkas.github.io/PLUTOpython/playground/)
- [Finish-Up-A-Thon arc page](https://stzifkas.github.io/PLUTOpython/arc/)
