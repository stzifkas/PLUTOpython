# Publishing pluto-ecss to PyPI

This file documents the exact steps to cut a release. Tests must be
green, the version in `pyproject.toml` and `src/pluto_ecss/__init__.py`
...
$EDITOR src/pluto_ecss/__init__.py # __version__ = "X.Y.Z"
...
rm -rf dist/ build/ src/pluto_ecss.egg-info/
...
pip install --force-reinstall dist/pluto-ecss-X.Y.Z-py3-none-any.whl
pluto-ecss --version
...
Then verify on https://pypi.org/project/pluto_ecss/ that:
...
- The console script entry point `pluto-ecss` is listed
...
1. Tag the release:
   ...
   ```
2. Create a GitHub release at https://github.com/stzifkas/pluto-ecss/releases/new from the new tag; attach the wheel and sdist from `dist/`.
3. Update the playground to install from PyPI instead of bundling source:
   - In `docs/playground/index.html`, replace the MEMFS write-loop with `await micropip.install("pluto-ecss")`.
...
```bash
twine upload dist/pluto-ecss-X.Y.Z+post1*    # bump to a post-release first
```
